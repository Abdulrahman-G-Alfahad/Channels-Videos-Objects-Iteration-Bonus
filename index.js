...
function channelWithMostContent(channels) {
  // Your code here
  const currentMost = channels[0];
  channels.forEach((channel) => {
    if (totalVideosDuration(channel) > totalVideosDuration(currentMost))
      currentMost = channel;
  });
  return currentMost;
}
...
function longestChannelName(channels) {
  // Your code here
  let currentLongest = channels[0];
  channels.forEach((channel) => {
    if (channel.name.length > currentLongest.name.length)
      currentLongest = channel;
  });
